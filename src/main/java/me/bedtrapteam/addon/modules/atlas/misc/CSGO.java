...
public class CSGO extends Module {
    ...
    private final Setting<anti_desync> spinbot_anti_decync = sg_spinbot.add(new EnumSetting.Builder<anti_desync>().name("anti-desync").defaultValue(anti_desync.All).build());
    private final Setting<Boolean> yaw = sg_spinbot.add(new BoolSetting.Builder().name("yaw").defaultValue(true).build());
    private final Setting<Boolean> pitch = sg_spinbot.add(new BoolSetting.Builder().name("pitch").defaultValue(false).build());
    private final Setting<Integer> yaw_speed = sg_spinbot.add(new IntSetting.Builder().name("yaw-speed").defaultValue(5).min(0).sliderMax(100).build());
    private final Setting<Integer> pitch_speed = sg_spinbot.add(new IntSetting.Builder().name("pitch-speed").defaultValue(5).min(0).sliderMax(100).build());
