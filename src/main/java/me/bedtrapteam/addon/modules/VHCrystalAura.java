...
public class VHCrystalAura extends Module {
    ...
    private final Setting<Boolean> thick = sgSwitch.add(new BoolSetting.Builder().name("thick").defaultValue(true).visible(render::get).build());
    ...
    public VHCrystalAura() {
        super(Categories.Combat, "VH-auto-crystal", "Automatically places and breaks crystals to damage other players.");
    }
    ...
    private void doPlace() {//TODO rework the order in which damages are calculated
        ...
        BlockIterator.register((int) Math.ceil(placeRange.get()), (int) Math.ceil(verticalRange.get()), (blockPos, blockState) -> {
            ...
            for (LivingEntity target : targets) {
                ...
                if ((blockState.getBlock() != Blocks.BEDROCK && blockState.getBlock() != Blocks.OBSIDIAN && (!canSupport || !blockState.getMaterial().isReplaceable() || supportDelayLeft > 0)) || !isEmpty(blockPos.add(0, 1, 0)))
                    return;
