...
public class TNTAura extends Module {
    ...
    private final Setting<Boolean> thickObsidian = sgObsidianRender.add(new BoolSetting.Builder().name("thick").defaultValue(true).build());
    ...
    private final Setting<Boolean> thickTnt = sgTNTRender.add(new BoolSetting.Builder().name("thick").defaultValue(true).build());
    ...
    private final Setting<Boolean> thickBreak = sgBreakRender.add(new BoolSetting.Builder().name("thick").defaultValue(true).build());
    ...
    public TNTAura() {
        super(Nigger.Category, "TNT-aura", "bedtrap tnt aura by ajaj");
    }
    ...
    private void onRender(Render3DEvent event) {
        ...
        if (obsidianRender.get() && !obsidianPos.isEmpty()) {
            for (BlockPos pos : obsidianPos) {
                ...
                if (thickObsidian.get()) {
                    Render2Utils.thick_box(event, pos, side, line, obsidianShapeMode.get());
                } else {
        ...
        if (tntRender.get() && target != null && allowTNT(target)) {
            if (tntBlockstate(target.getBlockPos().add(0, 2, 0))) {
                if (thickTnt.get()) {
                    Render2Utils.thick_box(event, target.getBlockPos().add(0, 2, 0), tntSideColor.get(), tntLineColor.get(), tntShapeMode.get());
                } else {
        ...
        if (breakRender.get() && target != null && autoBreak.get()) {
            if (!mineBlockstate(target.getBlockPos().add(0, 2, 0))) {
                if (thickBreak.get()) {
                    Render2Utils.thick_box(event, target.getBlockPos().add(0, 2, 0), breakSideColor.get(), breakLineColor.get(), breakShapeMode.get());
                } else {
